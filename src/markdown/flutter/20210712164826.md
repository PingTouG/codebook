...
- `maximumYear`：最大年份
- `minuteInterval`：分钟的粒度，必须是60的正整数因子，即一分钟可以分割为`60/minuteInterval`份，默认为`1`
- `use24hFormat`：是否使用24小时制，默认为`false`
- `backgroundColor`：背景色
...
- `minuteInterval`：分钟的粒度，必须是60的正整数因子，即一分钟可以分割为`60/minuteInterval`份，默认为`1`
- `secondInterval`：秒的粒度，必须是60的正整数因子，即一分钟可以分割为`60/secondInterval`份，默认为`1`

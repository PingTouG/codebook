...
- `floatingLabelBehavior`：焦点获取时label展示行为，默认为`FloatingLabelBehavior.auto`，可选值`FloatingLabelBehavior.always(总是显示)`，`FloatingLabelBehavior.never(从不显示)`

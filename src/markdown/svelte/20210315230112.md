...
`Svelte`通过`{}`来插入`JavaScript`变量，在`{}`中可以防止任何`JavaScript`代码，如：`name.toUpperCase()`。`{}`只能插入纯文本，如果需要插入`HTML`需要使用`{@html }`。`Svelte`还提供了一个调试 API：`{@debug val1,var2...}`，每当特定的值发生改变时都会记录这些变量的值，如果打开了调试控制台，则会暂停代码执行（打断点）。

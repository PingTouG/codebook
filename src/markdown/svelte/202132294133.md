...
在`Svelte`中响应式很简单，只需要定义一个变量，它就是响应式的，如：`记录点击按钮的次数`。
...
// 			let tmp = user
// 			tmp.github = 'https://github.com/PingTouG'
// 			user = tmp
			user.github = 'https://github.com/PingTouG'
		}
...
在`Svelte`中如果需要根据某个值来动态计算得到某个结果，并且值每次改变后，都会重新计算，那就需要用到`$:`(reactive declarations(响应式声明))，如：`通过姓和名得到全名`。

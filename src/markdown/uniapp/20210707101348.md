...
去[极光官网](https://www.jiguang.cn/)注册账号，然后创建一个应用，服务选择`消息推送`，在产品设置时填写的`应用包名`需要和开发的App包名一致，然后点击下一步就可以了，SDK不需要下载。
...
- 进入之后，在左侧菜单上点击`Identifiers`，点击进入对应的App
...
> 设置角标，即应用图标左上角数量，注意这个只是设置极光推送服务的角标，一般应用会根据推送中的`badge`自动设置应用角标，如果需要手动设置应用的角标需要使用`plus.runtime.setBadgeNumber(badge)`，如果需要清空角标可使用`plus.runtime.setBadgeNumber(0)`来清空，0表示清空
...
```js
const jv = uni.requireNativePlugin('JG-JPush')
jv.requestNotificationAuthorization((result)=>{
    if (result.status < 2) {
...
> 获取注册ID，需要连接成功之后获取，否则程序会崩溃

```js
const jv = uni.requireNativePlugin('JG-JPush')
jv.getRegistrationID()
```
...
```js
const jv = uni.requireNativePlugin('JG-JPush')
jv.openSettingsForNotification(result => {
    console.log(result)
})
```
